...
/* excerpts from md32_common.h ----------------------------------------*/

#define MD32_REG_T uint32_t
#define HOST_c2l(c, l)                      \
    (l = (((MD32_REG_T)(*((c)++))) << 24),  \
     l |= (((MD32_REG_T)(*((c)++))) << 16), \
     l |= (((MD32_REG_T)(*((c)++))) << 8), l |= (((MD32_REG_T)(*((c)++)))), l)
...
void sha256_block_data_order(SHA256_CTX *ctx, const void *in) {
    MD32_REG_T a, b, c, d, e, f, g, h, s0, s1, T1, T2, t;
    SHA_LONG X[16], l, Ki;
    int i;
...
void SHA256(const uint8_t *d, size_t n, unsigned char *md) {
    SHA256_CTX c;
    SHA256_Init(&c);
    SHA256_Update(&c, d, n);
    SHA256_Final(md, &c);
