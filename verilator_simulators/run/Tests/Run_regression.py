...
def do_isa_test (worker_no, args_dict, full_filename):
    ...
    command2 = [args_dict ['sim_path'],  "+trace", "+tohost", "+debug_port=666{0}".format(worker_no)]
    ...
    if os.path.exists ("./vcd/vlt_dump.vcd"):
        trace_filename = os.path.join (args_dict ['logs_path'], basename + ".vcd")
        os.rename ("./vcd/vlt_dump.vcd", trace_filename)
        message = message + ("    Wave output saved in: {0}\n".format (trace_filename))
